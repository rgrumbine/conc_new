...
////////////////////
    int outfreq = 10000000;
    if (nread % outfreq == 0) printf("nr = %.1f m\n",nread/1000000.); fflush(stdout);
    nread += 1;
  }
  fclose(fin);
...
// Now sweep the grid and wherever there are valid obs, compute ice concentration:
  ...
  for (loc.i = 0; loc.i < ngrid.xpoints(); loc.i++) {
     if (nh_hr_accum[loc].count == 0 || nh_lr_accum[loc].count == 0) {
       ...
       if (weather_accum(nh_lr_accum[loc], nh_hr_accum[loc]) == MAYBE) {
         ngrid[loc] = nasa_team2(nh_lr_accum[loc], nh_hr_accum[loc], arctic, (float) ll.lat);
       }
       else {
	 ngrid[loc] = weather_accum(nh_lr_accum[loc], nh_hr_accum[loc]);
       }
     }
  }
  }
...
// Southern hemisphere:
  ...
  for (loc.i = 0; loc.i < sgrid.xpoints(); loc.i++) {
     if (sh_hr_accum[loc].count == 0 || sh_lr_accum[loc].count == 0) {
       ...
       sgrid[loc] = nasa_team2(sh_lr_accum[loc], sh_hr_accum[loc], antarctic, (float) ll.lat);
     }
  }
  }
